...
impl BeautifyLines {
    ...
    fn print_header(&self) {
        ...
        let header_str = vec_str
            .get(0)
            .expect("We need at least one line of output to populate header");
    ...
    fn print_body(&self) {
        ...
        for i in self.start_ind..vec_str.len() {
            ...
            match &self.grep_word {
                Some(grep_word) => match line.find(grep_word) {
                    Some(start_ind) => {
                        let end_ind = start_ind + grep_word.len();

                        print!("{}", Self::custom_colorize(i, &line[0..start_ind]));

                        print!("{}", &line[start_ind..end_ind].blink());

                        print!("{}", Self::custom_colorize(i, &line[end_ind..]));

                        println!();
                    }
                    None => {
                        println!("{}", Self::custom_colorize(i, line));
                    }
                },
                None => {
                    println!("{}", Self::custom_colorize(i, line));
    ...
    fn get_max_len() -> usize {
        let max_len = term_size::dimensions()
            .expect("Couldn't get the terminal size")
            .0;
        max_len
    }
}
